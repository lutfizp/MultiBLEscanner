...
Bluetooth advertisements are radio broadcasts, but collecting them over time can reveal device presence, recurrence, and movement between monitored locations. Raw payloads, addresses, names, GATT serial values, scanner coordinates, timestamps, aliases, notes, and correlation decisions must be treated as operationally sensitive data.
...
Retention settings are present, but automatic retention cleanup is not currently implemented. Operators must monitor growth and establish an authorized cleanup process. Deletion work must preserve required audit events and must be verified against backup and legal-retention requirements.
